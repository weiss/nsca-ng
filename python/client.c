...
static pthread_rwlock_t nscang_client_instances_lock = PTHREAD_RWLOCK_INITIALIZER;


unsigned int set_psk( SSL *ssl, const char *hint,
                      char *identity, unsigned int max_identity_length,
                      unsigned char *psk, unsigned int max_psk_length)
{
  int rc;
...
int nscang_client_write( nscang_client_t *c, void *buf, int len, int timeout) {
  int rc;
  time_t endtime;
  fd_set fdset;
...
int nscang_client_response( nscang_client_t *c, int timeout) {
  int rc;
  time_t endtime;
  fd_set fdset;
  char buf[1024];
                             ...
                             int status, char *message, int timeout)
{
  ...
  if( service == NULL)
    len = snprintf( command, sizeof(command)-1, "[%u] PROCESS_HOST_CHECK_RESULT;%s;%d;%s",
                    (unsigned int) time(NULL), host, status, message);
  else
    len = snprintf( command, sizeof(command)-1, "[%u] PROCESS_SERVICE_CHECK_RESULT;%s;%s;%d;%s",
...
char * nscang_client_errstr( nscang_client_t *c, char *buf, int buf_size) {
  ...
  case NSCANG_ERROR_BAD_PROTO_VERSION:
    snprintf( buf, buf_size, "Protocol mismatch - bad version '%s'", c->errstr);
    break;
  case NSCANG_ERROR_PROTOCOL_MISMATCH:
    strncpy( buf, "Protocol mismatch - unexpected server response", buf_size);
    break;
  case NSCANG_ERROR_UNKNOWN_RESPONSE:
    strncpy( buf, "Protocol mismatch - unknown server response", buf_size);
    break;
  ...
  case NSCANG_ERROR_BAD_STATE:
    snprintf( buf, buf_size, "Operation not permitted in state %d", c->state);
    break;
